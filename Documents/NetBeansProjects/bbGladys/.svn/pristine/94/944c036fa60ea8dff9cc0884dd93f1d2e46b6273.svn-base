    ...
    public void GenerrarInformeStock() throws SQLException{
      HSSFWorkbook libro=new HSSFWorkbook();
       ...
       String sql="select *,(select articulos.NOMBRE from articulos where articulos.ID=movimientosarticulos.idArticulo)as descripcion from movimientosarticulos where idcaja="+Inicio.caja.getNumero();
        ...
        form=null;
        sql="SELECT id,cantidad,preciodecosto,preciodeventa,precioservicio,fecha,numerocomprobante,(select listcli.RAZON_SOCI from listcli where listcli.codMmd=movimientosarticulos.numeroCliente limit 0,1)as nombreC,(select articulos.NOMBRE from articulos where articulos.ID=movimientosarticulos.idArticulo limit 0,1)as descA,(select mozos.nombre from mozos where mozos.id=movimientosarticulos.numeroUsuario limit 0,1) as nombreU FROM movimientosarticulos where tipoMovimiento =1 and idcaja="+Inicio.caja.getNumero();
        //System.out.println(sql);
        //tra=new Conecciones();
