...
import RestoPantallas.AbmPedidos;
...
public class LoguinBbsGestion extends javax.swing.JFrame {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (usuarios.getNumero() > 0) {
            ...
            if (usuarios.getNivelDeAutorizacion() == 2) {
                //Color colorF = null;
                ArrayList lstMozos=new ArrayList();
                ArrayList lstMesas=new ArrayList();
                Restable resto=new MesasDao();
                Restable restoMo=new MozosDao();
                lstMozos = restoMo.ListarCVariables(null,0);
                Iterator it=lstMozos.listIterator();
                Mozos mozo;
                Mesas mesa;
                while(it.hasNext()) {
                    mozo=(Mozos)it.next();
                    int aa=mozo.getColor();
                    JFrame inicioF = new JFrame();
                    JDesktopPane jPanel = new JDesktopPane();
                    inicioF.getContentPane().add(jPanel);
                    ...
                    lstMesas=resto.Listar(mesa);
                    int x=30;
                    int y=30;
                    for (int ab = 0; ab < lstMesas.size(); ab++) {
                        mesa=(Mesas) lstMesas.get(ab);
                        AbmPedidos ingPed = new AbmPedidos(mesa,mesa.getIdMozo());
                        jPanel.add(ingPed);
                        //ingPed.setTitle("Mozo " + aa + " mesa - " + ab);
                        ingPed.setLocation(x, y);
                        //ingPed.setMaximum(false);
                    ...
                    inicioF.add(jPanel);
                    inicioF.setTitle("Mesas Mozo " + mozo.getNombre());
